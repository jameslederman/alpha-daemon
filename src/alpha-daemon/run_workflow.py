import asyncio
import argparse
from datetime import datetime, timezone
from temporalio.client import Client
from temporalio.contrib.pydantic import pydantic_data_converter
from temporalio import workflow
from uuid import uuid4

with workflow.unsafe.imports_passed_through():
    from workflows import ResearchWorkflow
    from models import ResearchRun, ResearchScope


def parse_as_of(value: str) -> datetime:
    try:
        dt = datetime.fromisoformat(
            value.replace("Z", "+00:00")
        )
    except ValueError as exc:
        raise argparse.ArgumentTypeError(
            "as-of must be an ISO 8601 datetime"
        ) from exc

    if dt.tzinfo is None:
        raise argparse.ArgumentTypeError(
            "as-of must include a timezone"
        )

    return dt


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()

    parser.add_argument(
        "symbol",
        help="Security ticker to research",
    )

    parser.add_argument(
        "--as-of",
        type=parse_as_of,
        default=None,
        help="Point-in-time knowledge cutoff",
    )

    return parser.parse_args()


def create_security_research_run(
    symbol: str,
    as_of: datetime | None = None,
) -> ResearchRun:
    created_at = datetime.now(timezone.utc)

    if as_of is None:
        resolved_as_of = created_at
    else:
        resolved_as_of = as_of.astimezone(timezone.utc)

    if resolved_as_of > created_at:
        raise ValueError("as_of cannot be in the future")

    return ResearchRun(
        run_id=f"research_run:{uuid4()}",
        scope=ResearchScope(
            kind="security",
            attributes={
                "symbol": symbol.strip().upper(),
            },
        ),
        as_of=resolved_as_of,
        created_at=created_at,
    )


async def main():
    args = parse_args()

    run = create_security_research_run(
        symbol=args.symbol,
        as_of=args.as_of,
    )

    # Connect client to the local temporal server
    client = await Client.connect(
        "localhost:7233",
        data_converter=pydantic_data_converter,
    )

    run_id = f"research_run:{uuid4()}"

    # Execute the workflow
    result = await client.execute_workflow(
        ResearchWorkflow.run,
        run,
        id=run.run_id,
        task_queue="my-task-queue",
    )

    print(f"Workflow result: {result}")

if __name__ == "__main__":
    asyncio.run(main())
